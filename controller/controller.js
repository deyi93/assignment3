...
exports.CreateTask = (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    if (appname === undefined || username === undefined || password === undefined || taskname === undefined) {
      console.log("appname cannot be empty")
      res.status(400).json({ code: 400 })
    } else {
                              ...
                              if (err1) {
                                ...
                                if (taskname === undefined || taskname === [""] || taskname === "" || taskname === " ") {
                                  ...
                                  if (taskname.length <= 45) {
                                            ...
                                            if (err) {
                                              ...
                                              connection.query(sqlCreate, (err2, result2) => {
                                                if (err2) {
                                                  ...
                                                  res.status(201).json({ code: 201, message: "Task Created Successfully", Task_Id: appname + "_" + id })
                                                }
                                              })
                                            }
                                          })
                                    ...
                                    console.log("cannot more than 45 charactor")
  ...
  if (!errors.isEmpty()) {
    ...
    dbConnect.getConnection(async (err, connection) => {
      if (err) {
        ...
        if (username === undefined || password === undefined || appname === undefined || taskid === undefined) {
          res.status(400).json({ code: 400 })
        } else {
          await connection.query(search_query, async (err, result) => {
            if (err) {
                        ...
                        if (data.length === 0) {
                          ...
                          connection.query(sqlSearch_grp, [username, data[0].App_permit_Doing], (err1, result) => {
                            if (err1) {
                              ...
                              if (result.length > 0) {
                                //check for task exist or not,else return 404
                                      ...
                                      await connection.query(sqlSearch_task, async (err2, result2) => {
                                        if (err2) {
                                          ...
                                          if (result2[0].Task_state === "doing") {
                                            ...
                                            console.log("task not in doing state")
                                            res.status(406).json({ code: 406 })
                                          }
                                        }
                                      })
...
exports.GetTaskbyState = (req, res) => {
  ...
  if (!errors.isEmpty()) {
    ...
    if (username === undefined || password === undefined || appname === undefined || state === undefined) {
      res.status(400).json({ code: 400 })
    } else {
      dbConnect.getConnection(async (err, connection) => {
        if (err) {
            ...
            if (err) console.log(err)
              ...
              if (result.length == 0) {
                ...
                if (await bcrypt.compare(password, hashedPassword)) {
                  if (result[0].status == 1) {
                    const accessToken = jwt.sign({ username }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: "15d" })
                    if (state === "open" || state === "todo" || state === "doing" || state === "done" || state === "close") {
